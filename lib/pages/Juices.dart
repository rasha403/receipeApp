...
class JuicesCategoryPage extends StatefulWidget {
  final String category;
  final String title;

  const JuicesCategoryPage({
    Key? key,
  ...
  @override
  State<JuicesCategoryPage> createState() => _JuicesCategoryPageState();
}

class _JuicesCategoryPageState extends State<JuicesCategoryPage> {
  ...
  Future<void> _loadRecipes() async {
    ...
    try {
      ...
      List<Map<String, dynamic>> allRecipes = await _dbHelper.getAllRecipes();
      print('Total recipes in DB: ${allRecipes.length}');
      
      // Debug: Check for juice recipes specifically
      List<Map<String, dynamic>> juiceRecipes = await _dbHelper.getRecipesByCuisineType('Juices');
      print('Juice recipes found: ${juiceRecipes.length}');
      
      // Debug: Print all recipe names and cuisine types
      for (var recipe in allRecipes) {
        print('Recipe: ${recipe['name']}, CuisineType: ${recipe['cuisineType']}');
      }
      
      setState(() {
        _recipes = juiceRecipes;
        _isLoading = false;
      });
    } catch (e) {
  ...
  Future<void> _showAddRecipeDialog() async {
    ...
    await showDialog(
      ...
      builder: (context) => AlertDialog(
        title: const Text('Add New Juice Recipe'),
  ...
  Future<void> _insertNewRecipe(String name, String ingredientsText, String chefName) async {
    try {
      ...
      Map<String, dynamic> recipeData = {
        ...
        'cuisineType': 'Juices',
        'difficulty': 'Easy',
        'prepTime': 5, // Default value
        'cookTime': 0, // Default value (no cooking needed)
...
class JuiceItemCard extends StatelessWidget {
